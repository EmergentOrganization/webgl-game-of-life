...
float s2 = 		getB(vec2(5.0, -4.0)) +
		...
		getB(vec2(11.0, 0.0)) +
		getB(vec2(11.0, 1.0)) +
		getB(vec2(11.0, 2.0));   /*NEWLINE*/ int sum_0 = int(s0 + s1 + s2); //EDGE_0
...
//:0,999,0,:77,133,1,:39,47,1,:11,11,1,
	...
	if(sum_0 >= 87 && sum_0 <= 133) 	{ gl_FragColor = vec4(0.2, 0.4, 1.0, 1.0); }
	if(sum_0 >= 54 && sum_0 <= 62) 		{ gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 32 && sum_0 <= 32) 		{ gl_FragColor = vec4(0.2, 0.8, 1.0, 1.0); }
	if(sum_0 >= 11 && sum_0 <= 11) 		{ gl_FragColor = vec4(1.0, 0.5, 1.0, 1.0); }
