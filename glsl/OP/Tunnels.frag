...
float s0 = 
		...
		getR(vec2(5.0, 1.0)) +
		getR(vec2(5.0, 2.0));
float s1 = 		getR(vec2(5.0, 5.0)) +
		getR(vec2(7.0, -3.0)) +
		getR(vec2(7.0, 0.0)) +
		getR(vec2(7.0, 3.0));   /*NEWLINE*/ int sum_0 = int(s0 + s1); //EDGE_0

	//restore last frame's data (current, a[my_id])		
	if(sum_0 >= 40) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 21 && sum_0 <= 26) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
