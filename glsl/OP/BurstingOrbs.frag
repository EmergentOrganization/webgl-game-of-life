...
float s6 = 		getR(vec2(10.0, 10.0)) +
		...
		getR(vec2(14.0, 2.0)) +
		getR(vec2(14.0, 3.0)) +
		getR(vec2(15.0, 0.0));   /*NEWLINE*/ int sum_0 = int(s0 + s1 + s2 + s3 + s4 + s5 + s6); //EDGE_0



	//restore last frame's data (current, a[my_id])
	if(sum_0 >= 158) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 100 && sum_0 <= 120) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
	if(sum_0 <= 76) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	//EDGE_2
