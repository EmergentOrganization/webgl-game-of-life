...
//Get gliders to be birthed from wave's crystal form
	...
	if(sum_0 >= 85 && sum_0 <= 89) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 113 && sum_0 <= 985) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
...
float s10 = 		getR(vec2(11.0, -9.0)) +
	...
	if(sum_2 >= 225) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_2 >= 265 && sum_2 <= 980) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }	
