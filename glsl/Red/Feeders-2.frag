...
//:126,999,0,:64,71,1,:71,121,1,:63,74,0,

	//restore last frame's data (current, a[my_id])

	if(sum_0 >= 126) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 64 && sum_0 <= 121) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }	
	if(sum_0 >= 63 && sum_0 <= 74) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 80 && sum_0 <= 85) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }	
