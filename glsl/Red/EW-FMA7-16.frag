...
//Get gliders to be birthed from wave's crystal form
	...
	if(sum_0 >= 32 && sum_0 <= 131) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	//The 42 band is too weak, replicators cannot repel or use their utility powerup
	if(sum_0 >= 42 && sum_0 <= 42) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 120 && sum_0 <= 190) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 68 && sum_0 <= 75) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }
...
float s10 = 		getR(vec2(11.0, -9.0)) +
	...
	if(sum_2 >= 265) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
