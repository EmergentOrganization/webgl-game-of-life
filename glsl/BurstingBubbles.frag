...
float s1 = 		getR(vec2(3.0, 3.0)) +
		...
		getR(vec2(6.0, 0.0)) +
		getR(vec2(6.0, 1.0)) +
		getR(vec2(8.0, 0.0));   /*NEWLINE*/ int sum_0 = int(s0 + s1); //EDGE_0


//:29,999,0,:21,24,1,:0,13,0,

	//restore last frame's data (current, a[my_id])

	if(sum_0 >= 29) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
	if(sum_0 >= 21 && sum_0 <= 24) { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }	
	if(sum_0 <= 13) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }
